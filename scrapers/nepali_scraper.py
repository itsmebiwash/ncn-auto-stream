...
# ── Groq output post-processors ──────────────────────────────────────────────

# Patterns in AI captions that should be stripped
...
def _clean_caption(text: str) -> str:
    """
    Post-process Groq fb_caption_text:
    1. Remove source link placeholders like [स्रोतको नाम].
    ...
    # Deduplicate sentences (split on ।  or . or \n)
    seen = set()
    deduped = []
...
def _clean_hashtags(hashtags: list) -> list:
    """
    Filter hashtag list to only include ASCII hashtags.
    Strips any Devanagari or other non-ASCII hashtags generated by the AI.
    """
    clean = []
    for tag in hashtags:
        tag = tag.strip()
        if not tag.startswith('#'):
            tag = '#' + tag
        # Allow only ASCII printable characters in hashtags
        if all(ord(c) < 128 for c in tag):
            clean.append(tag)
    # Always ensure at least these core tags
    if not clean:
...
def process_single_article(source_name, title, url, category="General"):
    ...
    # ── Clean Groq outputs before storing ─────────────────────
    caption_raw = groq_data.get("fb_caption_text", "")
    ...
    if image_url:
        # Use absolute paths so DB references always resolve regardless of CWD
        output_dir = os.path.abspath("output")
        os.makedirs(output_dir, exist_ok=True)
        bg_out = os.path.join(output_dir, f"{topic_slug}_bg.jpg")
        success, bg_path = optimize_image(image_url, bg_out, target_size=(1080, 1350))

        if success:
            final_out = os.path.join(output_dir, f"{topic_slug}.jpg")
            ...
            if rend_success:
                ...
                if opt_success:
                    final_image_path = os.path.abspath(path_or_err)
                    try:
                        os.remove(bg_path)
                    except Exception:
                        pass
